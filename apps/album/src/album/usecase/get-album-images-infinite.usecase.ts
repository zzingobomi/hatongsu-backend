...
@Injectable()
export class GetAlbumImagesInfiniteUseCase {
  constructor(
  ...
  ): Promise<AlbumMicroservice.AlbumImageInfiniteResponse> {
    const {
      nextCursor: reqNextCursor,
      //prevCursor: reqPrevCursor,
      limit,
    } = request;

    const [albumImages, { nextCursor }] =
      await this.albumImageDatabaseOutputPort.getAlbumImagesInfinite({
        nextCursor: reqNextCursor,
        //prevCursor: reqPrevCursor,
        limit,
      });
